...
export function AdminLayoutWrapper({
  ...
  if (!mounted) {
    return (
      <div className="bg-surface-subtle flex min-h-[calc(100vh-140px)] w-full">
        <aside className="bg-surface border-line fixed top-0 bottom-0 left-0 z-40 w-64 border-r" />
        <main className="flex-1 overflow-x-auto p-4 pl-64 sm:p-6 lg:p-10">
          {children}
        </main>
      </div>
    );
  ...
  return (
    <div
      ref={containerRef}
      className="bg-surface-subtle relative flex min-h-[calc(100vh-140px)] w-full"
    >
      <AdminSidebar collapsed={collapsed} onToggle={handleToggle} />
      <main
        className={`flex-1 overflow-x-auto p-4 transition-all duration-300 sm:p-6 lg:p-10 ${

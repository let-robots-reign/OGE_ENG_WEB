...
/**
 * Presentational activity heatmap + streak callout. Data-source-agnostic: the
 * student profile passes its own `getActivity`, the teacher view passes a
 * student's `getStudentActivity`. `readOnly` swaps the streak copy for a
 * neutral third-person label.
 */
export function ActivityView({
  data,
  readOnly = false,
}: {
  data: ActivityViewData | undefined;
  readOnly?: boolean;
}) {
                ...
                <div className="text-ink-3 mt-1.5 max-w-[220px] text-[13.5px] leading-[1.4]">
                  {readOnly
                    ? data.currentStreak === 0
                      ? "Пока нет активной серии занятий."
                      : data.isActiveToday
                        ? "Занимался сегодня — держит темп."
